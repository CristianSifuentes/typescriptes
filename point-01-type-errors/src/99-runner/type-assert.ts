 ...
 * PROBLEM
 * -------
 * A type is a *compile-time* object. By the time your program runs, every type
 * annotation has been erased. So when a tutorial writes
 *
 *     console.log("here TypeScript knows the value is a string");
 *
 ...
 * If the compiler's belief differs from the claim, the project fails to build.
 * The printed trace then becomes a *rendering* of a proved fact rather than a
 * promise.
 */
 ...
 * compares two *deferred* conditional types structurally, by identity of their
 * check type, rather than by mutual assignability. Mutual assignability would
 * wrongly report `any` as equal to everything, and `{a: string}` as equal to
 * `{a: string} & {}`. This encoding distinguishes `any`, `unknown`, `never`,
 * and each union member exactly.
 */
...
/**
 * The witness argument used by `proveType`. When the claimed type and the real
 * type disagree, the call site is required to supply a value of type `never`,
 * which no expression can produce. The compiler reports the shortfall as
 ...
 * program point.
 *
 *     const parsed = proveType<string>()(input, "string", "narrowed by typeof");
 *
 * - Compile time: `Actual` is inferred from `input`. If it is not *exactly*
 *   `string`, the extra `never` parameter becomes mandatory and the build
 *   breaks. The claim cannot rot.
