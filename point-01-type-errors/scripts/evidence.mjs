 ...
 * ----------------------
 * Every claim this project makes about compiler behaviour must be falsifiable.
 * `tsconfig.evidence.json` compiles a set of files that contain deliberate,
 * unsuppressed type errors; this script runs that compilation, parses the raw
 * diagnostics, and prints them grouped by file with their TSxxxx codes.
 *
...
/** `src/a/b.tsc-error.ts(12,7): error TS2322: Type 'x' is not ...` */
...
for (const line of raw.split(/\r?\n/)) {
  ...
  // Indented continuation lines are the compiler's ELABORATION chain: the
  // step-by-step reason the relation failed. For union types this is where the
  // real explanation lives ("...does not exist on type 'number'").
...
console.log(
  `\n${C.green}${C.bold}✔ ${total} type errors were caught at COMPILE TIME.${C.reset}`,
);
console.log(
  `${C.dim}  Zero of these programs ever reached Node. In JavaScript, each one\n` +
    `  would have been discovered by a user, in production, at 3 a.m.${C.reset}\n`,
);
