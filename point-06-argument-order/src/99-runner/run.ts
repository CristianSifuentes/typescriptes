 ...
 *     npm run demo:07-branded-types
 *
 * Usage (directly, after `npm run build`):
 *     node dist/99-runner/run.js 07-branded-types
...
function main(): void {
  ...
  if (requested === undefined || requested === "--list") {
    banner(
      ...
      "Concept #6 — compile-time prevention of argument-order mistakes",
    );
    table(
      ["id", "level", "granular feature"],
      demos.map((demo) => [demo.id, demo.level, demo.feature]),
    );
    note("Run one with:  npm run demo:<id>        e.g. npm run demo:07-branded-types");
