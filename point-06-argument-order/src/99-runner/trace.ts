 ...
 *
 * Types are erased at compile time (see 00-foundations/manifesto.md), which
 * means a type system is, by construction, *invisible* at runtime. This module
 * exists to make it visible: banners, before/after tables, "what does the
 * compiler believe here?" traces, and controlled runtime explosions.
 *
...
/**
 * Render what the compiler expects AT EACH ARGUMENT POSITION.
 *
 * Positional binding is invisible in source: `f(a, b)` looks symmetric, and
 * nothing on the page says "argument 0 is checked against parameter 0". This
 * helper makes that check visible — and, in the `verdict` column, shows
 * precisely where it has nothing to say (two parameters of the same type).
 */
export function positionTrace(
  ...
  table(["#", "parameter", "type demanded", "argument passed", "verdict"], rows);
}

/**
 * Render the *type state* the compiler holds at a given program point.
 *
 * Each row is `[location, expression, static type, why it changed]`.
 * The "static type" column is not decoration: in every demo that uses this
 * helper, the same fact is asserted with `proveType<T>()` or an
 * `Expect<Equals<...>>` alias, so if the compiler disagreed with the string
 * printed here the project would not compile.
 */
...
export function detonate(label: string, fragment: () => unknown): void {
  try {
    ...
    // `useUnknownInCatchVariables` forces us to prove what we caught before use.
    const description =
 ...
 *
 * Several demos deliberately contain expressions that TypeScript rejects (each
 * marked `@ts-expect-error`). Those lines are still emitted — `@ts-expect-error`
 * suppresses a diagnostic, it does not delete code — so executing them would
 * throw the very runtime error the demo is arguing you should never see.
 *
