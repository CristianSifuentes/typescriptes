...
export function runDemo(demo: Demo): void {
  banner(`DEMO ${demo.id} — ${demo.title}`, `${demo.level}  ·  ${demo.feature}`);
  ...
  console.log(
    `  ${colors.blue}${colors.bold}PART 2 — TYPESCRIPT${colors.reset}` +
      `${colors.grey}  (strict: true; diagnostics below were caught before this program ran)${colors.reset}`,
  );
  console.log(`  ${colors.grey}${"═".repeat(76)}${colors.reset}`);
  demo.safe();
  ...
  note(demo.codes.length > 0 ? demo.codes.join("  ·  ") : "(none — this demo documents a genuine gap; see its explanation.md)");
