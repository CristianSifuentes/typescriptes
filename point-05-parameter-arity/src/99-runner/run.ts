 ...
 *     npm run demo:11-overload-arity-resolution
 *
 * Usage (directly, after `npm run build`):
 *     node dist/99-runner/run.js 11-overload-arity-resolution
...
function main(): void {
  ...
  if (requested === undefined || requested === "--list") {
    banner("point-05-parameter-arity", "Concept #5 — compile-time enforcement of parameter arity and types");
    table(
      ["id", "level", "granular feature"],
      demos.map((demo) => [demo.id, demo.level, demo.feature]),
    );
    note("Run one with:  npm run demo:<id>        e.g. npm run demo:07-rest-parameters");
